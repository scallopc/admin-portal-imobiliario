...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log('Body recebido na API de import:', body)
    ...
    const prices = (body.units || [])
      .map((u) => {
        ...
        if (typeof v === 'string') {
          const n = parseFloat(v.replace(/\./g, '').replace(/,/g, '.').replace(/[^0-9.\-]/g, ''))
    ...
    console.log('ReleaseData final que será salvo:', releaseData)
