...
export async function getProperty(params: { id: string }): Promise<PropertyDTO> {
  ...
  const createdAt = migratedData.createdAt?.toDate
    ? migratedData.createdAt.toDate().toISOString()
    : (migratedData.createdAt ?? "");
  const updatedAt = migratedData.updatedAt?.toDate
    ? migratedData.updatedAt.toDate().toISOString()
